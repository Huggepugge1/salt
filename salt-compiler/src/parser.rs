...
impl Instruction for Statement {
    fn parse(parser: &mut Parser) -> Result<Self> {
        ...
        let kind = match token.kind {
            ...
            TokenKind::Ident(_) => Self::throw_parse_error(ParseErrorKind::UnexpectedToken {
                actual: token,
...
impl Parser {
    ...
    pub fn expect_ident(&mut self) -> Result<String> {
        ...
        }) = token
        {
            ...
            Err(ParseError::new(
                ParseErrorKind::UnexpectedToken(ExpectedToken::Specific(TokenKind::Ident(
                    String::new(),
                ))),
                Some(token),
            )
